...
public class WebsocketHandler extends TextWebSocketHandler{
	...
	public void setFriendService(FriendServiceMine friendService) {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		friendVO.setId1(map.get("myId").toString());
		friendVO.setId2(map.get("friendId").toString());
		String chatFile = friendService.checkChat(friendVO);
		if(chatFile == null) {
			System.out.println("새로생성");
			String fileName = System.currentTimeMillis() + ".txt";
			file = new FileWriter("D:/반응형웹개발자/Spring/springwork/project/src/main/webapp/chatLog/"+fileName,true);
			friendVO.setChat(fileName);
			friendService.updateChat(friendVO);
			friendVO.setId1(map.get("friendId").toString());
			friendVO.setId2(map.get("myId").toString());
			friendService.updateChat(friendVO);
		}else {
			System.out.println("이미 존재");
			BufferedReader reader = new BufferedReader(new FileReader("D:/반응형웹개발자/Spring/springwork/project/src/main/webapp/chatLog/"+chatFile));
			for(int i = 0; i<30; i++) {
